...
trait GameDef {
   ...
   * the game.
   */
  def startBlock: Block = Block(startPos, startPos) //Written By Me
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def neighbors: List[(Block, Move)] = List((down, Down),(up, Up),(right, Right),(left, Left)) //Written By Me

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter(p => p._1.isLegal) //Written By Me

    /**
     * Returns `true` if the block is standing.
     */
    def isStanding: Boolean = this.b1 == this.b2 //Written By Me

    /**
     * Returns `true` if the block is entirely inside the terrain.
     */
    def isLegal: Boolean = terrain(b2) && terrain(b1) //Written By Me
